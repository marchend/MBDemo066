...
final class LoginViewTests: XCTestCase {
    ...
    /// A `.configured` OktaConfig so the LoginViewModel's init-time
    /// `.notConfigured` pre-seed (added in MD066-2 PR 3) doesn't
    /// pollute tests that pre-date that feature.
    ...
    func test_signIn_closureTriggeredOnValidInput() {
        var triggered = false
        let vm = makeVM { _, _, _ in triggered = true }
        vm.username = "u"
        vm.password = "p"
        vm.signIn()
